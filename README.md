...
- 搭配 8 週主表的 28 天 AI 應用與資料整合 Course；每課依序完成一個主教材（影片或本站短文）、兩題理解題、專案轉譯題與面試實戰，通過後才解鎖下一天。
- 28 課加入論文、S-111／S-104、QGIS、NetCDF／HDF5、PostgreSQL API 與本專案同步機制的具體情境；頁面固定以「既有經驗／案例改寫／求職延伸」三類標示，避免把規劃內容說成既有成果。
- 理解題使用固定題目、標準答案與逐題解析；專案轉譯題與面試題沒有唯一文字答案，改以技術要點、驗證、風險與取捨 rubric 提供練習回饋。
- 部分課程提供經人工挑選的短影片作直覺補充；理解題仍以本站版本化摘要為準，避免影片下架、字幕或版本差異改變標準答案。
